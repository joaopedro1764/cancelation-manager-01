...
import { Building2, Calendar, AlertTriangle, ChevronRight, BarChart3, TrendingUp, TrendingDown } from "lucide-react";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";

export function TopCancelledCondosCard() {
    const [selectedMonth, setSelectedMonth] = useState("Maio");
    const [hoveredCondo, setHoveredCondo] = useState<string | null>(null);
    ...
    const contagemCondominios = cancelamentosFiltrados.reduce((acc, item) => {
        if (!item.condominio) {
            return acc;
        }
    ...
    const topCondominios = Object.entries(contagemCondominios)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 6);

    const totalCancelamentos = topCondominios.reduce((sum, [, quantidade]) => sum + quantidade, 0);

    // Simulação de dados do mês anterior para comparação
    const cancelamentosAnterior = Math.floor(totalCancelamentos * 0.88); // 12% a menos no mês anterior
    ...
    const getBarWidth = (quantidade: number) => {
        const max = Math.max(...topCondominios.map(([, q]) => q));
    ...
    const getRankingColor = (index: number) => {
        const colors = [
            ...
            { bg: "bg-slate-500", light: "bg-slate-50", text: "text-slate-700", border: "border-slate-200" }
        ];
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300 group overflow-hidden">
            {/* Header Premium */}
            <div className="relative bg-gradient-to-br from-blue-50 via-blue-50 to-indigo-50 p-4 border-b border-blue-100">
                {/* Background Pattern */}
                ...
                <div className="relative">
                    <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center space-x-3">
                            <div className="relative">
                                <div className="p-2.5 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg group-hover:shadow-xl transition-shadow">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-gray-800 mb-1">Condomínios Mais Cancelados</h3>
                                <p className="text-xs text-gray-600">Análise residencial - {selectedMonth}</p>
                            </div>
                        </div>
                        ...
                        <div className="text-right">
                            <div className="flex items-center space-x-2 mb-1">
                                <span className="text-2xl font-bold text-blue-600">{totalCancelamentos}</span>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm text-gray-600">Top 6 condomínios</span>
                        </div>
                        <div className="flex items-center space-x-2">
                            <Calendar className="h-4 w-4 text-gray-500" />
                            <Select value={selectedMonth} onValueChange={setSelectedMonth}>
                                <SelectTrigger className="w-32 h-8 text-sm bg-white/80 backdrop-blur-sm border-gray-200 hover:border-blue-300 focus:ring-blue-500/20">
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                    {meses.map(mes => (
            ...
            <div className="p-4">
                {topCondominios.length === 0 ? (
                    ...
                    <div className="space-y-2.5">
                        {topCondominios.map(([condominio, quantidade], index) => {
                            const colors = getRankingColor(index);
                            const percentage = ((quantidade / totalCancelamentos) * 100);
                            const isHovered = hoveredCondo === condominio;
                            ...
                            return (
                                <div 
                                    ...
                                    onMouseEnter={() => setHoveredCondo(condominio)}
                                    onMouseLeave={() => setHoveredCondo(null)}
                                >
                                    <div className="p-3.5">
